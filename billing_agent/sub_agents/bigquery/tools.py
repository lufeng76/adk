...
def get_bq_client():
    ...
    if bq_client is None:
        ...
        # Remove quotes if they exist in the project ID
...
def update_database_settings():
    ...
    # Get project ID and dataset ID, removing quotes if present
    project_id = get_env_var("BQ_PROJECT_ID")
    if project_id.startswith("'") and project_id.endswith("'"):
        project_id = project_id[1:-1]
...
**Think Step-by-Step:** Carefully consider the schema, question, guidelines, and best practices outlined above to generate the correct BigQuery SQL.
   ...
   """
    ...
    try:
        ...
        raise
        # litellm_response = litellm_model.llm_client.completion(
        #     model=litellm_model.model,
        #     messages=[{'role': 'user', 'content': prompt}],
        #     temperature=0.1
        # )
        # # Extract text from LiteLLM response which has a different format
        # sql = litellm_response.choices[0].message.content
